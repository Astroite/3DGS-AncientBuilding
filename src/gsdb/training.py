...
from .training_data import json_write, postshot_adapter, validate_package, validate_postshot_adapter
...
def train_package(package: Path, output: Path, backend='postshot', steps=None,
    ...
    python = Path(os.environ.get('GSDB_GSPLAT_PYTHON',str(app/'.venv-gsplat'/'Scripts'/'python.exe')))
    ...
    try:
        ...
        if backend=='postshot':
            ...
            evaluate = [str(python),'-m','gsdb.native_train','--dataset',str(package),

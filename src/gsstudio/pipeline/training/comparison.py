...
from gsstudio.infrastructure.persistence.run_repository import load_run, save_run
from gsstudio.pipeline.training.runner import train_package
from gsstudio.pipeline.training.data import prepare_segment, json_write
from gsstudio.infrastructure.runtime.run_lock import run_dir_locked
...
def compare_backends(run_dir: Path, segment: str, output: Path, *,
    ...
    if run.id != run_dir.name or not run.selected_dataset or run.stages['reconstruct'].status.value != 'succeeded':
        raise RuntimeError('Requires a completed reconstruction with segment QA')
    ...
    meta = prepare_segment(scene / run.selected_dataset, records, run.config.reconstruction.primary,
                           run.config.segment_qa, segment, package, duration_seconds=duration_seconds, reuse_files=True)

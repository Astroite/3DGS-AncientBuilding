"""项目树与 Run 状态、证据、日志和训练预览面板。

所有内容来自 :mod:`gsstudio.application`。界面只负责呈现：状态文字取自清单与阶段记录，
...
class ProjectTree(QTreeWidget):
    ...
    def select_context(self, location_id: str | None = None,
                       scene_id: str | None = None,
                       capture_id: str | None = None,
                       run_id: str | None = None) -> None:
        """Restore selection after a worker changes the manifest tree."""
        if not location_id:
            return
        for index in range(self.topLevelItemCount()):
            location = self.topLevelItem(index)
            if (location.data(0, ROLE_PAYLOAD) or {}).get("location") != location_id:
                continue
            target = location
            if scene_id:
                location.setExpanded(True)
                for child_index in range(location.childCount()):
                    child = location.child(child_index)
                    if (child.data(0, ROLE_PAYLOAD) or {}).get("scene") == scene_id:
                        target = child
                        break
                if target is not location and (capture_id or run_id):
                    target.setExpanded(True)
                    for group_index in range(target.childCount()):
                        group = target.child(group_index)
                        for item_index in range(group.childCount()):
                            item = group.child(item_index)
                            payload = item.data(0, ROLE_PAYLOAD) or {}
                            if ((capture_id and payload.get("capture") == capture_id) or
                                    (run_id and payload.get("run") == run_id)):
                                target = item
                                break
...
class RunDetailPanel(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None):
        ...
        self.qa_summary = QLabel("分段可训练：— · 全路线覆盖：—", objectName="caption")
        self.qa_report = QPlainTextEdit()
        ...
        layout.addWidget(self.qa_summary)
    ...
    def show_detail(self, detail: run_status.RunDetail) -> None:
        ...
        self.qa_summary.setText(
            f"分段可训练：{detail.training_status or '—'} · 全路线覆盖：{detail.coverage_status or '—'}"
        )
        self.qa_report.setPlainText(detail.qa_report_text or "QA 报告尚未生成。")

...
关键背景是 splatfacto 的 `stop_split_at = 15000`：**致密化在第 15,000 步就停止，之后的 85,000 步只是在精修一组固定的高斯**（009 最终 878,467 个，还低于 QA 的 2M 告警线，说明 splatfacto-big 的额外容量没被用上）。所以"100k 步是否必要"很可能有很大的下调空间，但这应当由数据决定。

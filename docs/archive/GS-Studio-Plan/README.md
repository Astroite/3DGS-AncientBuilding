...
本目录保存历史决策和证据。指向源码的链接已更新到本次重组后的文件位置，链接本身不表示历史版本的代码仍在运行。

本目录保存 2026-09-13 形成的下游训练/编辑产品计划、A 方向设计和技术评估，供后续实现追溯。**现行项目路线图在 [APP](../../architecture/GS-STUDIO-PROJECT-ROADMAP.md)，工程路线图也在 [APP](../../architecture/GS-STUDIO-ENGINEERING-ROADMAP.md)。**
...
本目录是 APP 仓库内的历史规划资料，不是第二套现行路线图。APP 与 Data 均未因路线图整合而移动；当前可运行原型仍在 APP。当前实际操作请使用 [APP 工作手册](../../user/CURRENT-WORKFLOW.md)。工作区顶层的 `GS-Studio-Plan` 原目录保留，本目录为可随 APP 提交的参考副本。
...
| 现行项目阶段与进入条件 | [APP 项目路线图](../../architecture/GS-STUDIO-PROJECT-ROADMAP.md) |
| 现行入口整合与工程顺序 | [APP 工程路线图](../../architecture/GS-STUDIO-ENGINEERING-ROADMAP.md) |

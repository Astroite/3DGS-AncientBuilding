...
更新：2026-09-22。本文规划 APP 原位升级的工程顺序，不是已经实施的架构或运行说明。项目阶段、需求与验收见[项目路线图](GS-STUDIO-PROJECT-ROADMAP.md)；现行命令和兼容边界以[工作手册](CURRENT-WORKFLOW.md)及[维护说明](MAINTENANCE.md)为准。

## 入口和业务边界

正式 GUI 以已选 [A 工作台设计](GS-Studio-Plan/design/specification-a/README.md)为基线，先补齐采集、Run、重建和 QA 页面。Python/gsdb 流水线与本地 gsplat 训练是主线；现有 Tk Studio 只作可复用功能线索。GUI 和 CLI 逐步调用同一业务服务，不通过解析 CLI 终端输出判定状态。正式 UI 框架在 P1 编码前单独定案。
...
| `preprocess` | Run 创建与阶段面板；新建/恢复、配置摘要、Run ID 和日志 | 新 Run schema 6；历史 Run 按原配置恢复 |
...
| `cleanup` | Run 存储面板；可回收空间、预览、确认、执行与 pending 重试 | 仅 schema 6，使用现行白名单、消费者校验和 Run 锁 |
...
| `postshot-prepare/review/train`、旧 `export`、`qa approve/reject`、`clean` | 历史项目的兼容查看/应急操作 | 不把旧发布验收链用于 schema 5/6 分段成果；旧命令保留兼容 |
...
1. **P1：入口与服务边界。** 盘点 Typer 命令中的参数解析、业务调用和展示逻辑；将可共享的操作、状态读取和错误分类整理为 GUI/CLI 共用服务。先交付只读项目浏览、依赖与日志视图，再接写入动作。具体进程、线程和 UI 框架由 P1 的架构决策记录确定。
...
状态来源须维持现行语义：Run `manifest.yaml` 与配置/输入哈希、遮罩最终清单、`selected_dataset`、分段 QA、实验 dispatch/训练记录/评估及清理日志共同决定可显示的结果。错误状态保留，不能手改清单或绕过锁、校验、QA、retention 消费者检查。schema 1～5 的读取与恢复、新 schema 6 默认行为均保持兼容。

## 实施前决策与验证关口

| 关口 | 最迟阶段 | 必须定下的内容 |
| --- | --- | --- |
| UI 技术选型与部署 | P1 编码前 | A 设计的视口、缩放、中文路径、Windows 本地打包及资源协作方案 |
| D-05 项目存储 | P1 写入项目前 | 外部源引用/可选归档、重定位与身份校验，避免原型隐式全量复制成为默认 |
| D-06 分组来源与兼容 | P3 导入/训练前 | 已有可信分组的保留和普通 COLMAP 缺失分组时的可见行为 |
| D-07 拾取语义 | P4 编辑前 | 非穿透可见性的用户可观察规则；沿用已接受的手柄＋数值交互 |
| D-08/D-10 质量目标与预设 | P3/P5 前 | 训练预设、代表素材、画质和资源判据；不得把原型默认或短训当正式结论 |

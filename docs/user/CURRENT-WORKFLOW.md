...
```powershell
$AppRoot = 'E:\Projects\3DGS\GSStudio'   # 改为实际工程路径
...
```powershell
$env:GSSTUDIO_REALITYSCAN_CLI = 'C:\Program Files\Epic Games\RealityScan_2.2\RealityScan.exe'
$env:GSSTUDIO_REALITYSCAN_EXPORT_PARAMS = Join-Path $AppRoot 'tools\realityscan-setup\colmap-export-params.xml'
$env:GSSTUDIO_POSTSHOT_CLI = 'C:\Program Files\Jawset Postshot\bin\postshot-cli.exe'
$env:INSTA360_MEDIA_SDK_ROOT = Join-Path $AppRoot 'sdks\Insta360-MediaSDK'
$env:GSSTUDIO_MEDIA_HELPER = Join-Path $AppRoot 'tools\mediasdk-helper\build\Release\gsstudio-media-helper.exe'
# CUDA 工具链不在标准位置时设置 GSSTUDIO_CUDA_HOME；自定义训练解释器时设置 GSSTUDIO_GSPLAT_PYTHON。
```
...
$SceneDir = Join-Path (Join-Path $env:GSSTUDIO_DATA_ROOT $LocationId) $SceneId

Invoke-Gsstudio location init $LocationId --name '地点名称'
Invoke-Gsstudio scene init $LocationId $SceneId --name '场景名称'
Invoke-Gsstudio capture init $LocationId $SceneId $CaptureId `
...
Invoke-Gsstudio media probe $LocationId $SceneId $CaptureId
Invoke-Gsstudio ingest $LocationId $SceneId $CaptureId
```
...
```powershell
Invoke-Gsstudio preprocess $LocationId $SceneId $CaptureId
$RunId = '<复制实际输出的 RUN_ID>'
$RunDir = Join-Path $SceneDir $RunId
Invoke-Gsstudio mask $LocationId $SceneId $RunId
Invoke-Gsstudio reconstruct $LocationId $SceneId $RunId
Invoke-Gsstudio qa report $LocationId $SceneId $RunId
```
...
# 实际原生训练；先完成代表片段检查。
Invoke-Gsstudio train $LocationId $SceneId $RunId --segment $SegmentId `
...
Invoke-Gsstudio preprocess $LocationId $SceneId $CaptureId --run-id $RunId --resume
Invoke-Gsstudio mask $LocationId $SceneId $RunId --resume
Invoke-Gsstudio reconstruct $LocationId $SceneId $RunId --resume
Invoke-Gsstudio qa report $LocationId $SceneId $RunId --resume
...
```powershell
# 只预览，不删除数据。
Invoke-Gsstudio cleanup $LocationId $SceneId $RunId
# 重试已满足依赖的清理，自动清理通常无需手动调用。
Invoke-Gsstudio cleanup $LocationId $SceneId $RunId --apply
...
```powershell
Invoke-Gsstudio mask-finalize $LocationId $SceneId $RunId --attempt primary
# 仅出现 repair 人工确认门禁时执行：
Invoke-Gsstudio mask-finalize $LocationId $SceneId $RunId --attempt repair
Invoke-Gsstudio reconstruct $LocationId $SceneId $RunId --resume
```
...
& $GsstudioPython (Join-Path $AppRoot 'scripts\compare-backends-v5.py') `
...
`Invoke-Gsstudio select-sharp` 在 Run 流水线之外，用同一套抽帧、打分、投影和遮罩代码从**显式路径**的原片里挑最清晰帧并剔除人物遮挡严重的视图，用来在正式登记之前判断一段素材值不值得做。不写 Capture/Run 清单，不训练，不是画质验收路径。

```powershell
Invoke-Gsstudio select-sharp --source $Source --out 'D:\work\pick' `
